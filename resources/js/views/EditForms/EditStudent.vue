                    ...
                    >
                        <header class="text-center">
                            <div class="sm:flex items-baseline">
                                <h2
                                    class="sm:pl-14 basis-[94%] text-lg md:text-2xl font-bold uppercase mb-3"
                                >
                                    Edit details of student with ID:
                                    <span
                                        class="text-blue-500 underline underline-offset-2"
                                    >
                                        {{ $route.params.student_id }}</span
                                    >
                                </h2>
                                <div
                            ...
                            <p class="text-sm sm:text-base mb-4 mt-3">
                                Edit details of the student or click
                                <router-link
                                    :to="{ name: 'Dashboard' }"
...
export default {
    ...
    methods: {
        ...
        async onSubmit(e) {
            this.updatePayload.formFields = this.formFields;
            this.updatePayload.id = this.$route.params.student_id;
            e.preventDefault();
            await this.editStudentEntry(this.updatePayload);
            if (Object.keys(this.allErrors).length == 0) {
                sessionStorage.setItem(
                    "CURRENT_SEM",
                    this.updatePayload.formFields.current_sem
                );
                sessionStorage.setItem("STARTING_SEM", 1);
                this.$router.push({
                    name: "EditStudentFamily",
                    params: { student_id: this.updatePayload.id },
                });
            }
        },
    ...
    async created() {
        ...
        for (const key in this.formFields) {
            if (this.formFields.hasOwnProperty(key)) {
                this.formFields[key] = this.singleStudent[key];
            }
        }
        this.formFields.fsc_marks = this.formFields.fsc_marks * 1100;
        this.formFields.matric_marks = this.formFields.matric_marks * 1100;
    },
    data() {
        return {
            updatePayload: {
